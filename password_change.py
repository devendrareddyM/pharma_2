            ...
            with transaction.atomic():
                ...
                if Permission[USER_TYPE].iloc[
                    ...
                    if self._request_payload[LOGIN_ID] in log_in_id:
                        obj = HashingSalting()
                        if obj.check_password(self._request_payload[USERPASSWORD_KEY], Permission[SALT_KEY].iloc[0],
                            ...
                            if ENV:
                                ...
                                self._psql_session.execute(
                                    CHANGE_RESET_USER_PASSWORD_QUERY.format(SALT_KEY, salt_value,
                                                                            USERPASSWORD_KEY,
                                                                            hash_value,
                                                                            LOGIN_ID,
                                                                            self._request_payload[
...
def change_user_password(request):
    ...
    try:
        ...
        excMsg = "change_password API : " + str(error_instance(e))
        return excMsg
    finally:
        if obj:
            del obj
